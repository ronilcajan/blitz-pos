...
const expenseDataLength = computed(() => {
    if (Object.keys(route().params).length > 0) {
        return props.expenses.data.length + 1;
    }
    
    return props.expenses.data.length;
})
...
<template>
    ...
    <TitleContainer :title="title">
        <div v-if="expenseDataLength !== 0" class="flex items-center gap-2 flex-wrap justify-end">
            <CreateBtnLink href="expenses/create" >New expenses</CreateBtnLink>
            <StatusFilter v-model="status" />
        </div>
    </TitleContainer>

    <EmptyContainer :title="title" v-if="expenseDataLength === 0">
        <CreateBtnLink href="expenses/create">New expenses</CreateBtnLink>
    </EmptyContainer> 

    <div v-if="expenseDataLength > 0" class="flex-grow">
            ...
            <div class="card-body grow-0">
                <div class="flex justify-between gap-2 flex-col-reverse sm:flex-row">
                    <div class="flex gap-2 flex-col sm:flex-row">
                        ...
                        <SelectDropdownFilter v-if="categories.length" v-model="category" :url="url" :title="`categories`"
                            :options="categories" />
