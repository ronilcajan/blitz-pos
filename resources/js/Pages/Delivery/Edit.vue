...
const deliveryForm = useForm({
    supplier_id: props.delivery.supplier_id,
	transaction_date: new Date(props.delivery.created_at).toISOString().split('T')[0],
    purchase_id : props.delivery?.purchase_id,
...
const submitDeliveryForm = () => {
    if(deliveries.length === 0) {
        useToast().error('Please add product to for the delivery!', {
            position: 'top-right',
            duration: 3000,
            dismissible: true
        });
	...
	deliveryForm.patch(`/deliveries/${props.delivery.id}`,
    {
		replace: true,
		preserveScroll: true,
  		onSuccess: () => {
			useToast().success(`Delivery has been updated successfully!`, {
				position: 'top-right',
				duration: 3000,
				dismissible: true
			});
...
const tx_no = props.delivery.purchase?.tx_no;
        ...
        <div class="w-full">
            <div class="card bg-base-100 shadow-sm">
                <div class="card-body grow-0">
                    ...
                    <div class="flex xl:gap-4 flex-col md:flex-row">
                        <div>
                            <InputLabel for="date" class="label" value="Purchase Orders"/>
                            <TextInput v-model="tx_no" readonly type="text" class="input input-sm w-full mb-1" required/>
                        </div>
                        <div>
                    ...
                    <div class="flex justify-end gap-2 flex-col lg:flex-row">
                            <div class="flex justify-end gap-3 flex-col md:flex-row">
                                <NavLink href="/deliveries" class="btn btn-sm">
